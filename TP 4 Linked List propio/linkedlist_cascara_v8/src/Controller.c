...
int controller_addEmployee(LinkedList* pArrayListEmployee)
{
    ...
    if(pArrayListEmployee != NULL)
    {
        ...
        if(newEmployee != NULL)
        {
            ...
            printf("\n\nDesea incorporar al empleado al final de la lista o en una posicion a eleccion? ");
            printf("\n1) Al final de la lista\n");
            printf("2) En un lugar a eleccion\n");
            printf("\n\nMarque su opcion: ");
            scanf("%d", &opcion);
            ...
            else
            {
                ...
                printf("Marque el lugar donde va a guardar el nuevo empleado: ");
                scanf("%d", &index);
                while(index > tam || index < 1)
                {
                    printf("Seleccione un indice posible: ");
                    scanf("%d", &index);
                }
                index = index - 1; //el usuario no escribe del 0 al 999, sino del 1 al 1000

                if(!ll_push(pArrayListEmployee, index, newEmployee))
                {
                    printf("\n\n Alta del empleado con exito. Nuevo empleado cargado:\n\n ");
                    printf("**********************************************\n");
                    printf("ID\t Nombre      Horas Trabajadas Sueldo\n");
...
int controller_clearList(LinkedList* pArrayListEmployee)
{
    ...
    if(pArrayListEmployee != NULL)
    {
        printf("Esta seguro/a de que desea borrar todos los elementos de la lista?\n ");
        printf("1) Si, proceda\n");
...
LinkedList* controller_cutList(LinkedList* pArrayListEmployee)
{
    ...
    if(pArrayListEmployee != NULL)
    {
        printf("La lista va del 1 al %d. De donde hasta donde quiere cortar?\n", tam);
        printf("Desde: ");
        scanf("%d", &from);
        while(from > tam || from < 1)
        {
            printf("Incorrecto, elija un numero posible: ");
            scanf("%d", &from);
        }
        from = from - 1;
        ...
        printf("\nHasta: ");
        scanf("%d", &to);
        while(to > tam || to < 0 || to < from)
        {
            printf("Incorrecto, elija un numero posible: ");
            scanf("%d", &to);
        }

        listaCortada = ll_subList(pArrayListEmployee, from, to);
    }
...
int controller_removeByIndex(LinkedList* pArrayListEmployee)
{
    ...
    {
        printf("Esa posicion no es posible, elija otra: ");
        scanf("%d", &index);
    }
    index = index - 1;

    emp = ll_pop(pArrayListEmployee, index);

    if(emp != NULL)
    {
        if(!ll_contains(pArrayListEmployee, emp)) //valido con contains que haya sido efectivamente borrado
        {
            printf("\n*** Empleado borrado ***\n");
            employee_print(emp);
            todoOk = 1;
